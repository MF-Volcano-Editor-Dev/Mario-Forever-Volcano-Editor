...
func _enter_tree() -> void:
	_level_events.level_completed.connect(_on_level_completed)
	_level_events.level_completion_stopped.connect(_on_level_stopped_completion)
...
func _on_level_completed() -> void:
	...
	_level_events.level_to_be_completed_state(0)
	...
	if scene_after_finish:
		Scenes.jump_to_packed(scene_after_finish, true, reserve_characters_data) # Changes the scene
	else:
		printerr("[Scene Changing Error] The scene to be warped to is invalid or not set yet!") # Throws an error on failure
	
	_level_events.level_done_completion() # Notices the level's completion
#endregion
