class_name CharacterBehavior2DNew extends Component
...
@export_subgroup("Shapes","shape_")
@export var shape_body_transform_normal: Transform2D = Transform2D(0.0, Vector2(11, 14), 0.0, Vector2(0, 2))
@export var shape_body_transform_crouch: Transform2D = Transform2D(0.0, Vector2(11, 14), 0.0, Vector2(0, 2))
@export var shape_head_position_normal: Vector2 = Vector2(0, -13)
@export var shape_head_position_crouch: Vector2 = Vector2(0, -13)
...
func _controls(player: CharacterEntity2D) -> void:
	...
	var uncontrollable: bool = ObjectState.is_state(player, CharacterEntity2D.STATE_UNCONTROLLABLE)
	_on_jump = Input.is_action_just_pressed(key_jump + id) if !uncontrollable else false
	_on_jump_held = Input.is_action_pressed(key_jump + id) if !uncontrollable else false
	_on_running = Input.is_action_pressed(key_run + id) if !uncontrollable else false
	_on_climbing = Input.is_action_just_pressed(key_climb + id) if !uncontrollable else false
#endregion


#region == Shape ==
func _shape(player: CharacterEntity2D) -> void:
	# Crouching shape transforms
	if is_crouching(player):
		if player.shape.transform != shape_body_transform_crouch:
			player.shape.transform = shape_body_transform_crouch
		if player.head_shape.position != shape_head_position_crouch:
			player.head_shape.position = shape_head_position_crouch
	# Non-crouching shape transforms
	else:
		if player.shape.transform != shape_body_transform_normal:
			player.shape.transform = shape_body_transform_normal
		if player.head_shape.position != shape_head_position_normal:
			player.head_shape.position = shape_head_position_normal
#endregion
