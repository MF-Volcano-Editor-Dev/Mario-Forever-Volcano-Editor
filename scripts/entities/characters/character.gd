...
static var _character_getter: CharacterGetter = CharacterGetter.new()
...
## Returns a in-script global class, [Character.CharacterGetter] which is used to get character(s).
static func get_character_getter() -> CharacterGetter:
	return _character_getter
...
class CharacterGetter:
	...
	func get_characters(scene_tree: SceneTree) -> Array[Character]:
	...
	func get_character(scene_tree: SceneTree, character_id: int) -> Character:
	...
	func get_nearest(scene_tree: SceneTree, to_gpos: Vector2) -> Character:
		var characters := get_characters(scene_tree)
	...
	func get_farest(scene_tree: SceneTree, to_gpos: Vector2) -> Character:
		var characters := get_characters(scene_tree)
		var distances: Array[float] = [] # The packed array doesn't support max() nor min()
		for i in characters:
			distances.append(i.global_position.distance_squared_to(to_gpos))
	...
	func get_random(scene_tree: SceneTree, amount: int) -> Array[Character]:
		var characters := get_characters(scene_tree)
		var result: Array[Character] = []
		for i in amount:
