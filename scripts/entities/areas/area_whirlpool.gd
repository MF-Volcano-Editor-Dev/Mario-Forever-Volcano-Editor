...
@export var particles_material_rect: ParticleProcessMaterial = preload("res://assets/particles/bubble_whirlpool_rect.tres")
@export var particles_material_center_oriented: ParticleProcessMaterial = preload("res://assets/particles/bubble_whirlpool_radial.tres")
...
func _physics_process(delta: float) -> void:
	var radius := shape_owner_get_shape(0 ,0).get_rect().size.length() / 2
